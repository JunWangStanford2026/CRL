...
class MiniLM(nn.Module):
    '''
        ...
        outputs probabilities in [0, 1], shape (B, N)
    '''
...
class MiniLMHRA(nn.Module):
    '''
        embedding (w/ positional encodings) -> transformer blocks -> (multiple heads) linear layer -> softmax
        outputs probabilities in [0, 1], shape (B, num_output_heads, vocab_size)
    '''
    def __init__(self, vocab_size, embedding_dim, hidden_dim, num_output_heads):
        super().__init__()
        self.vocab_size = vocab_size
        self.embedding_dim = embedding_dim
        self.hidden_dim = hidden_dim
        ...
        encoder_layer = nn.TransformerEncoderLayer(d_model=embedding_dim, nhead=2, dim_feedforward=hidden_dim)
        self.transformer_encoder = nn.TransformerEncoder(encoder_layer, num_layers=2)
        self.linear = nn.Linear(embedding_dim, num_output_heads * vocab_size)
    ...
    def forward(self, x):
        ...
        assert transformer_output.shape == (B, x.size(1), self.embedding_dim)
        transformer_output = transformer_output + embedded  # residual connection
        output = self.linear(transformer_output[:, -1, :]).view(B, self.num_output_heads, self.vocab_size)  # take the last token's output, apply output heads and unravel their outputs, shape (B, num_output_heads, vocab_size)
        return torch.softmax(output, dim=-1)
